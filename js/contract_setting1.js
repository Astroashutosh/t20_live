const main_contract = "0x752d949d135C764D0b45336b5d9EDFec77dac97c";
...
async function getLevelBusiness(level) {
    ...
    try {
        ...
        const data = await mainContract.methods
            .getLevelBusiness(account, level)
            .call();

        $(".leftBusiness1").text(`$ ${price(data.left)}`);
        $(".rightBusiness1").text(`$ ${price(data.right)}`);
        $(".totalLeftBusiness").text(`$ ${price(data.totalleft)}`);
        $(".totalRightBusiness").text(`$ ${price(data.totalright)}`);
...
async function loadBinaryTree(userAddress) {
    ...
    try {
        ...
        // Root
        $("#rootNode").html(nodeHtml(account));

        // Root Binary
        const root = await mainContract.methods.userBinary(account).call();

        $("#leftNode").html(nodeHtml(root.leftChild));
        $("#rightNode").html(nodeHtml(root.rightChild));

        // Left Side
        if (root.leftChild) {

            const left = await mainContract.methods
                .userBinary(root.leftChild)
                .call();

            $("#leftLeftNode").html(nodeHtml(left.leftChild));
            $("#leftRightNode").html(nodeHtml(left.rightChild));
        ...
        if (root.rightChild) {
            ...
            $("#rightLeftNode").html(nodeHtml(right.leftChild));
            $("#rightRightNode").html(nodeHtml(right.rightChild));
...
function nodeHtml(address, active = true) {
