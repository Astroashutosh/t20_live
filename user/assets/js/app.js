...
function initTheme() {
  let saved = null;
  try { saved = localStorage.getItem('veri-theme'); } catch (e) { /* storage unavailable */ }
  ...
  document.querySelectorAll('[data-action="toggle-theme"]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      try { localStorage.setItem('veri-theme', next); } catch (e) { /* storage unavailable */ }
    });
  });
}
...
function initScrollReveal() {
  ...
  // Fail-safe: never let content stay invisible. If the observer somehow
  // doesn't fire (blocked script, timing edge case, etc.), force reveal.
  setTimeout(() => {
